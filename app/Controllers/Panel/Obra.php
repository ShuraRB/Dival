<?php 
    ...
    class Obra extends BaseController{
        ...
        public function __construct(){
            ...
            if (acceso_usuario(TAREA_OBRA)) {
                $session->tarea_actual = TAREA_OBRA;
            }//end if 
            else{
        ...
        public function index(){
            ...
            if($this->permitido){
                // dd($this->permitido);
                return $this->crear_vista("panel/obra", $this->cargar_datos());
        ...
        private function cargar_datos(){
            ...
            $datos['nombre_pagina'] = 'Obra';

            //Cargamos el modelo correspondiente
            $tabla_obra = new \App\Models\Tabla_obra;
            $datos['obra'] = $tabla_obra->data_table_obra($session->id_obra);

            return $datos;
        }//end cargar_datos

        private function crear_vista($nombre_vista, $contenido = array()){
            $contenido['menu'] = crear_menu_panel(TAREA_OBRA, '');
        ...
        public function eliminar($id_obra = 0) {
            //Cargamos el modelo correspondiente
            $tabla_obra = new \App\Models\Tabla_obra;
            //Query
            ...
            if (!empty($obra)) {
                ...
                if($tabla_obra->delete($id_obra)) {
                    mensaje("El obra ha sido eliminado exitosamente", SUCCESS_ALERT);
                }//end if eliminar
                else {
                    mensaje("Hubo un error al eliminar a este obra, intenta nuevamente", DANGER_ALERT);
                }//end else

            }//end if count
            else {
                mensaje("El obra que deseas eliminar no existe", WARNING_ALERT);
            }//end else count
